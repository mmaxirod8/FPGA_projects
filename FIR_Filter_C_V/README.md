...
* **`Octave testing/`**: MATLAB/Octave scripts (`.m`) used to generate the test stimuli and mathematically verify the algorithm (open them with NotePad++ or text editor). More specific:
    ...
    * `sharp_image_filter.m`: Implementa el algoritmo de realce de bordes mediante filtros horizontales y verticales, guardando el resultado visual en formato JPG.
    * `write_ascii_ppm.m`: Función auxiliar que convierte los datos de una imagen a un archivo de texto ASCII (PPM), permitiendo que otros programas (como los testbenches) lean los píxeles fácilmente.
